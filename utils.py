...
def prepare_parser():
  ...
  parser.add_argument(
    '--num_standing_accumulations', type=int, default=8,
  ...
  parser.add_argument(
    ...
    '--base_root', type=str, default='/home/fk1/fahad/data/imagenet/single_cate/single_cate_places/alley_hdf/alley1000_hdf',
  ...
  parser.add_argument(
    '--weights_root', type=str, default='',
    help='Default location to store weights (default: %(default)s)')
  parser.add_argument(
    '--logs_root', type=str, default='',
    help='Default location to store logs (default: %(default)s)')
  parser.add_argument(
    '--samples_root', type=str, default='',
...
def add_sample_parser(parser):
  parser.add_argument(
    ...
    help='Sample "sample_num_npz" images and save to npz? '
         '(default: %(default)s)')
  parser.add_argument(
    '--sample_num_npz', type=int, default=16,
...
def save_weights(G, D, M, state_dict, weights_root, experiment_name, 
...
def load_weights(G, D, M, state_dict, weights_root, experiment_name, 
                 name_suffix=None, G_ema=None, strict=False, load_optim=False):
  #root = '/'.join([weights_root, experiment_name])
  root = 'D:\p84\MineGAN\data\weights'
  if name_suffix:
  ...
  #if M is not None:
  #  M.load_state_dict(
  #    torch.load('%s/%s.pth' % (root, join_strings('_', ['M', name_suffix]))),
  #    strict=strict)
  #  if load_optim:
  #    M.optim.load_state_dict(
  #      torch.load('%s/%s.pth' % (root, join_strings('_', ['M_optim', name_suffix]))))
  # Load state dict
...
def sample(G, M, z_, y_, config):
  with torch.no_grad():
    ...
    if config['parallel']:
      ...
      # yaxing
      z_from_M, y_from_M = M(z_)
      G_z = G(z_from_M, y_from_M)
      #G_z = G(z_, G.shared(y_))
    return G_z, y_


# Sample function for sample sheets
def sample_sheet(G,M, classes_per_sheet, num_classes, samples_per_class, parallel,
  ...
  for i in range(num_classes // classes_per_sheet):
    ...
    for j in range(samples_per_class):
      ...
      with torch.no_grad():
        if parallel:
          z_from_M, y_from_M =  nn.parallel.data_parallel(M, (z_[:classes_per_sheet],))
          o =  nn.parallel.data_parallel(G, (z_from_M, y_from_M))
          #o = nn.parallel.data_parallel(G, (z_[:classes_per_sheet], G.shared(y)))
        else:
          z_from_M, y_from_M = M(z_[:classes_per_sheet])
          o = G(z_from_M, y_from_M)
